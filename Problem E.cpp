...
const map<int64_t, int64_t> &count(vector<int64_t> seq, vector<int64_t> prev, bool save)
{
    ...
    if (save)
    {
      ...
      while (curs.size() < curc.size() + 30)
      {
        ...
        for (int i = 0; i < curc.size(); i++)
          x += curs[curs.size() - curc.size() + i] * curc[i];
        curs.push_back(x);
      }
  ...
  if (seq.size() >= 2)
  {
    ...
    if (it == memo.end() || !it->second.count(seq.back()))
      return empty;
  }
  ...
  for (int64_t c = 1; c <= seq[0]; c++)
    for (int64_t a = 1; a * c <= seq[0]; a++)
    {
      ...
      if (save)
      {
        curc.insert(curc.begin(), c);
        cura.insert(cura.begin(), a);
      }
      for (auto [v, n] : count(seq, prev, save))
        ret[v + tmp * c] += n;
      if (save)
      {
        curc.erase(curc.begin());
    ...
    while (seq.size() < 30 && seq.back() < 1e16)
    {
      ...
      for (auto [v, n] : m)
      {
        ...
        else
        {
          seq.push_back(v);
          if (n <= 20)
            goto done; // Small enough to brute force.
          break;
        }
      }
    }
